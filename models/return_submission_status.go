...
func NewReturnSubmissionStatus(value ReturnSubmissionStatus) *ReturnSubmissionStatus {
	v := value
	return &v
}
...
func init() {
	...
	if err := json.Unmarshal([]byte(`["accepted","limit_check_pending","limit_check_failed","limit_check_passed","released_to_gateway","delivery_confirmed","delivery_failed","submitted","validation_pending"]`), &res); err != nil {
		panic(err)
	}
	for _, v := range res {
...
func (m ReturnSubmissionStatus) validateReturnSubmissionStatusEnum(path, location string, value ReturnSubmissionStatus) error {
	if err := validate.EnumCase(path, location, value, returnSubmissionStatusEnum, true); err != nil {
		return err
	}
	return nil
}
...
// ContextValidate validates this return submission status based on context it is used
func (m ReturnSubmissionStatus) ContextValidate(ctx context.Context, formats strfmt.Registry) error {
	return nil
}
