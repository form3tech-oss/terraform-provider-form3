...
func resourcePayportAssociationCreate(d *schema.ResourceData, meta interface{}) error {
	...
	name := d.Get("payport_account_name").(string)
...
func resourcePayportAssociationRead(d *schema.ResourceData, meta interface{}) error {
	...
	associationName := d.Get("payport_account_name").(string)
	...
	d.Set("payport_account_name", association.Payload.Data.Attributes.PayportAccountName)
	d.Set("payport_account_id", association.Payload.Data.Attributes.PayportAccountUID)
	return nil
}
...
func resourcePayportAssociationDelete(d *schema.ResourceData, meta interface{}) error {
	...
	log.Printf("[INFO] Deleting association for id: %s account name: %s", associationFromResource.ID, associationFromResource.Attributes.PayportAccountName)
...
func createAssociationFromResourceData(d *schema.ResourceData) (*models.Association, error) {
	...
	if attr, ok := d.GetOk("payport_account_name"); ok {
		association.Attributes.PayportAccountName = attr.(string)
	}

	if attr, ok := GetUUIDOK(d, "payport_account_id"); ok {
		association.Attributes.PayportAccountUID = attr
	}
